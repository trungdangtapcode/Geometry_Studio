...
test("locks and unlocks all keyed tracks on an object layer", async ({ page }) => {
  ...
  const layerLock = cubeGroup.locator(".timeline-group-lock");
  await expect(layerLock).toBeEnabled();
  await layerLock.click();
  await expect(layerLock).toHaveClass(/active/);
  await expect(page.locator('.timeline-track-label[data-track-kind="position"]').first()).toHaveClass(/locked-track/);
  await expect(page.locator('.timeline-track-label[data-track-kind="rotation"]').first()).toHaveClass(/locked-track/);
  await expect(page.locator('.timeline-track-label[data-track-kind="scale"]').first()).toHaveClass(/locked-track/);

  const lockedScene = await saveScene(page);
  const lockedTracks = selectedTransformTracks(lockedScene);
  expect(lockedTracks.map((track) => [track.kind, track.locked])).toEqual([
    ["position", true],
    ["rotation", true],
    ["scale", true]
  ]);
  ...
  await layerLock.click();
  await expect(layerLock).not.toHaveClass(/active/);
  await expect(page.locator('.timeline-track-label[data-track-kind="position"]').first()).not.toHaveClass(/locked-track/);
  expect(errors).toEqual([]);
});

function selectedTransformTracks(scene: SceneExport): Array<{ kind: string; locked: boolean }> {
  ...
  return objectTimeline?.tracks
    ...
    .map((track) => ({ kind: track.kind, locked: track.locked }))
