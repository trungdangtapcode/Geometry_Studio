...
test("solos and unsolos all keyed tracks on an object layer", async ({ page }) => {
  ...
  const layerSolo = cubeGroup.locator(".timeline-group-solo");
  await expect(layerSolo).toBeEnabled();
  await layerSolo.click();
  await expect(layerSolo).toHaveClass(/active/);
  await expect(cubeGroup).toHaveClass(/solo-layer/);
  await expect(page.locator('.timeline-track-label[data-object-id="object-1"][data-track-kind="position"]').first()).toHaveClass(/solo-track/);
  await expect(page.locator('.timeline-track-label[data-object-id="object-1"][data-track-kind="rotation"]').first()).toHaveClass(/solo-track/);
  await expect(page.locator('.timeline-track-label[data-object-id="object-1"][data-track-kind="scale"]').first()).toHaveClass(/solo-track/);

  const soloScene = await saveScene(page);
  const soloTracks = selectedTransformTracks(soloScene);
  expect(soloTracks.map((track) => [track.kind, track.solo])).toEqual([
    ["position", true],
    ["rotation", true],
    ["scale", true]
  ]);
  ...
  await layerSolo.click();
  await expect(layerSolo).not.toHaveClass(/active/);
  await expect(cubeGroup).not.toHaveClass(/solo-layer/);
  await expect(page.locator('.timeline-track-label[data-object-id="object-1"][data-track-kind="position"]').first()).not.toHaveClass(/solo-track/);
  expect(errors).toEqual([]);
});

function selectedTransformTracks(scene: SceneExport): Array<{ kind: string; solo: boolean }> {
  ...
  return objectTimeline?.tracks
    ...
    .map((track) => ({ kind: track.kind, solo: track.solo }))
