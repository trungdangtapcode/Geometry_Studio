...
test("copies and pastes keyframe values without changing timing", async ({ page }) => {
  ...
  await setTimelineTime(page, 0);
  await runCommand(page, "copy keyframe value", "timeline.copy-value");

  await page.keyboard.press("Escape");
  await setTimelineTime(page, 2);
  await runCommand(page, "paste keyframe value", "timeline.paste-value");
  ...
  expect(positionTrack?.keyframes.map((keyframe) => keyframe.time)).toEqual([0, 2]);
  expect(positionTrack?.keyframes.map((keyframe) => keyframe.value[0])).toEqual([1, 1]);
  expect(positionTrack?.keyframes.map((keyframe) => keyframe.interpolation)).toEqual(["linear", "linear"]);
