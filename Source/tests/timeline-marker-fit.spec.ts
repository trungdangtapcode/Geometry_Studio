...
type SceneExport = {
  ...
  timeline: {
    ...
    markers: Array<{ label: string; time: number }>;
  };
};

test("fits selected keyframe block between neighboring markers", async ({ page }) => {
  ...
  await page.locator("#timeline-marker-label").fill("Start Beat");
  await page.locator("#timeline-add-marker").click();
  await setTimelineTime(page, 7);
  await page.locator("#timeline-marker-label").fill("End Beat");
  await page.locator("#timeline-add-marker").click();
  await expect(page.locator(".timeline-marker")).toHaveCount(2);

  await page.keyboard.press("Control+A");
  await setTimelineTime(page, 5);
  await runCommand(page, "fit keyframes between neighbor markers", "timeline.fit-between-markers");
  ...
  expect(scene.timeline.markers.map((marker) => ({ label: marker.label, time: marker.time }))).toEqual([
    { label: "Start Beat", time: 3 },
    { label: "End Beat", time: 7 }
  ]);
