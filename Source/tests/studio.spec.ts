...
test("auto-key pose records position rotation and scale together", async ({ page }) => {
  test.setTimeout(120_000);
  const errors: string[] = [];
  await page.addInitScript(() => {
    const downloads: string[] = [];
  ...
  await page.goto("/");
  await page.locator("#timeline-auto-key").check();
  await page.locator("#timeline-auto-key-pose").check();
  await expect(page.locator("#keyframe-dock")).toHaveClass(/auto-key-pose-active/);
  ...
  const sceneText = await page.waitForFunction(() => (window as unknown as { __sceneDownloads?: string[] }).__sceneDownloads?.at(-1) ?? null);
  const sceneDocument = JSON.parse((await sceneText.jsonValue()) as string);
  expect(sceneDocument.timeline.autoKeyPose).toBe(true);
  const objectTimeline = sceneDocument.timeline.objects.find((object: { objectId: string }) => object.objectId === "object-1");
  const track = (kind: string) => objectTimeline.tracks.find((candidate: { kind: string }) => candidate.kind === kind);
  expect(track("position").keyframes.map((keyframe: { time: number; value: number[] }) => [keyframe.time, keyframe.value[0]])).toEqual([[0, 0], [1, 4]]);
  expect(track("rotation").keyframes.map((keyframe: { time: number; value: number[] }) => [keyframe.time, keyframe.value[1]])).toEqual([[0, 0], [1, 90]]);
...
test("retimes multiple selected keyframes from the key editor anchor time", async ({ page }) => {
  test.setTimeout(120_000);
  const errors: string[] = [];
  await page.addInitScript(() => {
    const downloads: string[] = [];
  ...
  const objectTimeline = sceneDocument.timeline.objects.find((object: { objectId: string }) => object.objectId === "object-1");
  const positionTrack = objectTimeline.tracks.find((track: { kind: string }) => track.kind === "position");
  expect(positionTrack.keyframes.map((keyframe: { time: number }) => keyframe.time)).toEqual([3, 4]);
  expect(errors).toEqual([]);
});

test("stretches multiple selected keyframes from the key editor span", async ({ page }) => {
  test.setTimeout(120_000);
  const errors: string[] = [];
  await page.addInitScript(() => {
    const downloads: string[] = [];
  ...
  const objectTimeline = sceneDocument.timeline.objects.find((object: { objectId: string }) => object.objectId === "object-1");
  const positionTrack = objectTimeline.tracks.find((track: { kind: string }) => track.kind === "position");
  expect(positionTrack.keyframes.map((keyframe: { time: number }) => keyframe.time)).toEqual([1, 5]);
  expect(errors).toEqual([]);
});
...
test("keeps playback stopped after repeated Stop activations", async ({ page }) => {
  test.setTimeout(120_000);
  const errors: string[] = [];
  page.on("console", (message) => {
    if (message.type() === "error") errors.push(message.text());
  ...
  await page.locator("#play-toggle").evaluate((button) => {
  ...
  await expect(page.locator("#timeline-play-toggle")).toContainText("Stop 1x");
  await expect.poll(async () => Number(await page.locator("#timeline-current-time").inputValue())).toBeGreaterThan(0.05);
  await page.locator("#timeline-play-toggle").evaluate((button) => {
    (button as HTMLButtonElement).click();
    (button as HTMLButtonElement).click();
  });
  await expect(page.locator("#play-toggle")).toContainText("Play");
