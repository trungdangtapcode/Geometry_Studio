...
test("moves selected keyframe block to the nearest marker", async ({ page }) => {
  ...
  await page.locator("#timeline-marker-label").fill("Beat 1");
  await page.locator("#timeline-add-marker").click();
  await expect(page.locator(".timeline-marker")).toHaveCount(1);

  await page.keyboard.press("Control+A");
  await runCommand(page, "move keyframes to nearest marker", "timeline.move-to-nearest-marker");
