...
8. Click the shield button on the object timeline row, or search
   `Toggle Selected Object Layer Lock` in Commands, when an object is finished
   and should not be renamed, transformed, duplicated, deleted, parented,
   re-materialed, or keyed by accident. Click/run it again to unlock. This is
   different from the timeline group lock switch, which only protects keyed
   tracks.
