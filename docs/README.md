...
- [In-App Quick Help](in-app-quick-help.md) documents the searchable Help
  overlay, category filters, `?` shortcut, command-palette entries, and browser
  validation.
...
- [Timeline Visible Row Selection](timeline-visible-row-selection.md) documents
  selecting keyframes from the current row filter/search result, including
  before/after playhead tail-edit selections.
...
- Object layer enable switches mute or restore every keyed track on a layer in
  one action while reusing the existing per-track enable runtime behavior.
- Object layer solo switches isolate every keyed track on a layer in one action
...
- Visible-row before/after playhead selection commands support tail-edit
  workflows after narrowing rows with search or filters.
