...
texture mapping, animation, scene persistence, and an evaluator-focused tour.
The application is built with Three.js, TypeScript, Vite, and a keyframe
timeline editor, and is packaged as a static release that can be served locally
for grading.
\end{abstract}
...
and a light sweep animation. The renderer uses \texttt{PCFSoftShadowMap} and a
shadow-receiving ground plane to make shadows visible during grading.
...
configured with sRGB color space. Repeat values on the X and Y axes can be
...
The application supports both preset animation modes and a transform keyframe
timeline. Per-object preset animation modes include:
\begin{itemize}
  \item Spin
  \item Orbit
  \item Bounce
...
The project also includes a light sweep animation. Animation updates use elapsed
time and frame delta values so that motion remains stable across displays with
different refresh rates.

The keyframe timeline provides a bottom editing dock with a playhead, timecode,
duration, FPS, Work In/Out range, snapping, loop control, and tracks for object
transforms, object appearance, camera properties, and lights. Users can add,
delete, duplicate, copy, paste, drag, scrub, save, and reload keyframes.
...
without deleting keyframes. Frame-accurate navigation is available through
previous/next frame buttons and keyboard shortcuts, while selected keyframes can
be nudged one frame left or right for precise retiming. The editor stores
rotation keyframes in readable Euler degrees and converts transform animation to
Three.js playback tracks where appropriate.
...
\begin{itemize}
  ...
  \item Display settings such as grid, axes, stats, and frustum helper
...
At runtime, timeline tracks are compiled into native Three.js animation clips:
\begin{itemize}
  \item Position and Scale use \texttt{VectorKeyframeTrack}.
  \item Rotation uses \texttt{QuaternionKeyframeTrack}.
  \item Each keyed object is evaluated by an \texttt{AnimationMixer}.
\end{itemize}
...
\caption{Transform keyframe timeline showing two Position keyframes and a
selected object whose X position was keyed through the timeline workflow.}
\end{figure}
...
animation/timelineEditing.ts & Pure keyframe editing operations such as source resolution, copy, paste, duplicate, and frame nudge. \\
animation/clipFactory.ts & Converts timeline tracks into Three.js keyframe clips. \\
animation/timelinePlayer.ts & Evaluates timeline clips with \texttt{AnimationMixer}. \\
...
renderer/pipeline.ts & WebGLRenderer, EffectComposer, OutlinePass, and resize handling. \\
animation/timeline.ts & Animation update logic. \\
...
space, soft shadows, and post-processing outline selection. Resize handling uses
the canvas client size and caps the drawing buffer pixel count to avoid excessive
GPU load on high-DPI displays.
...
The current release focuses on a stable real-time WebGL editor. Future work
should improve rendering quality while preserving interactivity and grading
clarity.

\begin{enumerate}
  \item \textbf{Rendering Lab}: add tone mapping, exposure, shadow quality, PBR
  material presets, environment lighting controls, and a clearer rasterization
  versus ray tracing explanation.
  \item \textbf{Environment Lighting}: use local environment assets and
  \texttt{PMREMGenerator} to support physically-based reflections and
  roughness-dependent lighting.
  \item \textbf{OBJ + MTL Import}: extend OBJ import so users can load companion
  material files and texture images for models such as teacups, furniture, or
  scanned objects.
  \item \textbf{Post-Processing}: add controlled toggles for bloom, ambient
  occlusion, color grading, and anti-aliasing passes.
  \item \textbf{Timeline Polish}: add auto-key, duplicate keyframes, camera
  tracks, and light/material tracks.
...
an Evaluation Tour, screenshot export, and a modular source architecture. These
features make the project easier to evaluate, easier to maintain, and more
