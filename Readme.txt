...
- Asset Browser: left-rail dock with Online Models from Khronos/OS3A, one-click Load Campus landscape from copied school-campus GLB assets, a Campus tab for individual local assets, plus Built-in looks, procedural textures, materials, Teapot/Torus Knot geometry, and the Sample Drone model.
...
- Display helpers: Blender-style UI Density, UI Scale, reversible Clean View for screenshots (`Alt+G`, hides grid, axes, transform gizmo, helpers, overlays, and blur effects), Grid, Axes, FPS, selected-object outline, object and camera motion paths, onion-skin pose ghosts, progress UI, and toast messages.
